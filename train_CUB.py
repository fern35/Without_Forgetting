"""Launches routines that train (fewshot) recognition models on CUB

Example of usage:
(1) For our proposed approach proposed:

    # 1st training stage: trains a cosine similarity based recognition model.
    CUDA_VISIBLE_DEVICES=0 python train.py --config=cub_Conv128CosineClassifier
    ...
    # generator:
    CUDA_VISIBLE_DEVICES=0 python train.py --config=cub_Conv128CosineClassifierGenWeightAttN1 # 1-shot case.
    CUDA_VISIBLE_DEVICES=0 python train.py --config=cub_Conv128CosineClassifierGenWeightAttN5 # 5-shot case.

    All the configuration files that are used when launching the above
    training routines (i.e., cub_Conv128CosineClassifier.py,
    cub_Conv128CosineClassifierGenWeightAttN1.py, and
    cub_Conv128CosineClassifierGenWeightAttN5.py) are placed on the
...
(2) For our implementations of the Matching Networks and Prototypical networks
    ...
    CUDA_VISIBLE_DEVICES=0 python train.py --config=cub_Conv128MatchingNetworkN1

    # Train the matching networks model for the 5-shot case.
    CUDA_VISIBLE_DEVICES=0 python train.py --config=cub_Conv128MatchingNetworkN5

    # Train the prototypical networks model for the 1-shot case.
    CUDA_VISIBLE_DEVICES=0 python train.py --config=cub_Conv128PrototypicalNetworkN1

    # Train the prototypical networks model for the 5-shot case.
    CUDA_VISIBLE_DEVICES=0 python train.py --config=cub_Conv128PrototypicalNetworkN5
"""
...
from dataloader import CUB, FewShotDataloader
...
dataset_train = CUB(phase=train_split)
dataset_test = CUB(phase=test_split)
