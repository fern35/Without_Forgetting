"""Evaluates a fewshot recognition models on CUB

The routines evaluates the model that has achieved the best so far accuracy on
the recognition of novel categories on the validation set of CUB.

Example of usage on evaluating the proposed approach model already trained with
train routine:

# Evaluating the model for the 1-shot case on the test set of CUB.
CUDA_VISIBLE_DEVICES=0 python evaluate.py --config=cub_Conv128CosineClassifierGenWeightAttN1 --testset

# Evaluating the model for the 5-shot case on the test set of CUB.
CUDA_VISIBLE_DEVICES=0 python evaluate.py --config=cub_Conv128CosineClassifierGenWeightAttN5 --testset
...
parser.add_argument('--evaluate', default=False, action='store_true',
    help='If True, then no training is performed and the model is only '
         'evaluated on the validation or test set of CUB.')
parser.add_argument('--num_workers', type=int, default=4,
    help='number of data loading workers')
parser.add_argument('--cuda', type=bool, default=True, help='enables cuda')
parser.add_argument('--testset', default=False, action='store_true',
    help='If True, the model is evaluated on the test set of CUB. '
...
# recognition accuracy on the validation set of CUB
